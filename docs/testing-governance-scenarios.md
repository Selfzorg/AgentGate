...
- approvals, async evidence tasks, dry-run evidence, execution tokens, runner attempts, execution logs, and audit events are stored in PostgreSQL;
...
| Approval action | `production_deploy` | `REQUIRE_APPROVAL` | pending approval packet, queued evidence tasks, passed gate checks after evidence completion, approval grant, credential issue, execution queue, runner completion |
...
- queue and complete read-only evidence tasks before approval readiness;
