...
const DEFAULT_SHELL_TOOLS = new Set(["Bash"]);
const DEFAULT_EDIT_TOOLS = new Set(["Edit"]);
const DEFAULT_WRITE_TOOLS = new Set(["Write"]);
const DEFAULT_PATCH_TOOLS = new Set();

export function normalizeToolEvent(event, env = process.env, config = {}) {
  ...
  const supported = isSupportedTool({
    hookEventName,
    toolName,
    normalizedToolName,
    supportedTools: config.supportedTools ?? new Set()
  });

  const redactedToolInput = redactValue(toolInput);
  const rawAction = rawActionForTool(toolName, normalizedToolName, redactedToolInput, event, config);
  ...
  const safety = classifyActionSafety({
    toolName: safetyToolName(toolName, config),
...
function isSupportedTool({ hookEventName, toolName, normalizedToolName, supportedTools }) {
  return (
    hookEventName === DEFAULT_HOOK_EVENT_NAME &&
    (supportedTools.has(toolName) || String(toolName).startsWith("mcp__") || String(normalizedToolName).startsWith("mcp."))
  );
}

function rawActionForTool(toolName, normalizedToolName, toolInput, event, config) {
  const shellTools = config.shellTools ?? DEFAULT_SHELL_TOOLS;
  const editTools = config.editTools ?? DEFAULT_EDIT_TOOLS;
  const writeTools = config.writeTools ?? DEFAULT_WRITE_TOOLS;
  const patchTools = config.patchTools ?? DEFAULT_PATCH_TOOLS;
  ...
  if (patchTools.has(toolName)) {
    const filePath = stringFrom(toolInput.file_path, toolInput.path, event?.file_path, "");
    return filePath ? `apply_patch ${filePath}` : "apply_patch";
  }

  if (String(toolName).startsWith("mcp__") || String(normalizedToolName).startsWith("mcp.")) {
    return `${normalizedToolName}(${safeJsonStringify(toolInput ?? {})})`;
  }

  return stringFrom(event?.raw_action, toolInput.raw_action, normalizedToolName, toolName, "");
}
...
function safetyToolName(toolName, config) {
  const shellTools = config.shellTools ?? DEFAULT_SHELL_TOOLS;
  return shellTools.has(toolName) ? "Bash" : toolName;
}
