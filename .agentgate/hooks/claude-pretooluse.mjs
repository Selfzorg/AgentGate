...
import { normalizeClaudeEvent } from "./lib/normalize-claude-event.mjs";
import { redactValue, safeJsonStringify } from "./lib/redact.mjs";

export async function runHookEvent(event, env = process.env, options = {}) {
  const normalized = normalizeClaudeEvent(event, env);

  if (!normalized.supported) {
    const output = allowOutput("AgentGate ignored unsupported Claude hook event.", {
      decision: "ALLOW",
      mode: "observe",
      supported: false
    });
...
function allowOutput(reason, agentgate) {
  return claudeOutput("allow", reason, agentgate);
}

function denyOutput(reason, agentgate) {
  return claudeOutput("deny", reason, agentgate);
}

function claudeOutput(permissionDecision, reason, agentgate) {
...
async function maybeWriteDebugLog(entry, env, options) {
  ...
  const logPath = env.AGENTGATE_HOOK_LOG_PATH ?? join(projectRoot, ".agentgate", "logs", "hook-events.jsonl");
